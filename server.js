...
// ---- merging the AI's readings ----
// The document is read 2-3 times and the results are combined, so a chapter missed by one reading is still found.
// Chapter names are matched loosely ("Chapter 1 - Cell", "Chapter1: The Cell", "Ch 1 Cell" and a cut-off
// "Chapter 1 - The C" are all the same chapter), so the same chapter is never added twice.
const SUBJECT_ALIASES = { maths: "mathematics", math: "mathematics", sst: "social science", "social studies": "social science", evs: "environmental studies", bio: "biology", chem: "chemistry", phy: "physics", eng: "english", "computer science": "computer", "computer applications": "computer", comp: "computer", it: "information technology", hin: "hindi", skt: "sanskrit" };
function subjectKey(name) {
  const k = normalizeName(name).replace(/[^a-z0-9 ]/g, " ").replace(/\s+/g, " ").trim();
...
// When the student already has a subject/chapter, reuse its exact name, so confirming the import adds only
// genuinely new chapters instead of a second copy spelled slightly differently.
function alignToExisting(subjects, exams) {
  const list = Array.isArray(exams) ? exams : [];
  return subjects.map((s) => {
    const ex = list.find((e) => subjectKey(e.subject) === subjectKey(s.subject));
    if (!ex) return s;
    const existing = (Array.isArray(ex.chapters) ? ex.chapters : []).map((c) => ({ text: String(c.name || ""), parts: topicParts(c.name) }));
    const topics = s.topics.map((t) => { const p = topicParts(t); const hit = existing.find((o) => sameTopic(o.parts, p)); return hit ? hit.text : t; });
    return { ...s, subject: ex.subject, topics: [...new Set(topics)] };
  });
}
...
function mergeExtractionPasses(passResults) {
  ...
  passResults.forEach((subjects) => {
    subjects.forEach((s) => {
      ...
      if (!bySubject.has(key)) {
        bySubject.set(key, { subject: s.subject, examDateVotes: {}, examTimeVotes: {}, examNameVotes: {}, topics: [], seen: 0 });
      }
      const entry = bySubject.get(key);
      entry.seen += 1;
      if (s.subject.length > entry.subject.length && s.subject.length <= 40) entry.subject = s.subject; // "Mathematics" over "Maths"
  ...
  return Array.from(bySubject.values()).map((entry) => {
    ...
    return {
      ...
      topics: dedupeTopics(entry.topics),
    };
  });
}
...
app.post("/api/import/analyze", importLimiter, requireAuth, async (req, res) => {
  try {
    ...
    let merged = mergeExtractionPasses(succeeded);
    try { merged = alignToExisting(merged, (await studyContext(req.userId)).exams); } catch (e) { console.error("align import error", e.message); }
    if (merged.length === 0) {
